<?php 
// https://dev.omegatheme.com/group-price-attribute/cron/update_collections.php?action=updateCollection&shop=sajaro-invitations.myshopify.com
...
if($action == "updateCollection"){
    $collections = db_fetch_array("select collection_id from quantity_product where shop = '".$shop."' and collection_id != '' GROUP BY collection_id");
    foreach($collections as $collection){
        $collection_id = $collection['collection_id'];
        $result = getCollectionByID($collection_id,$shopify);
        var_dump($result);
    }
} 
 
function getCollectionByID($id,$shopify){
    $collection = $shopify("GET", "/admin/custom_collections/{$id}.json?&fields=id,title");
    return $collection;
}
...
function getPriceGroupByID($id,$shop){
    $result = db_fetch_row("select percent,number from price_groups  where shop = '$shop'");  
