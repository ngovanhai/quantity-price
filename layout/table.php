...
$html .= '<table class="priceGroupList">
            ...
            foreach ($listOfferOfVariant as $offerOfVariant) {
                ...
                $html .= '<tr>';
                    $html .= '<td>'.$settings['table_text1'].'  <span class="otNumber"> '.$offerOfVariant['number'].' </span> '.$settings ['table_text'].' </td>';
                    if($settings['showColumnLayoutTable'] == 0){
                        if($settings['show_percent'] != 0){
                            $html .= '<td class="">'.$discountPerProduct.' '.$settings['offLabel'].'</td>';
                        }else{
